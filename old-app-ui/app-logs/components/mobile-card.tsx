...
export const MobileCard = <T,>({ diffs, formatDisplayValue, copyToClipboard }: MobileCardProps<T>) => {
    const t = useLang();

    if (diffs.length === 0) {
        return <p className='p-2 text-[10px] text-gray-500 dark:text-gray-400'>{t.helper('nothing_action')}</p>;
    }

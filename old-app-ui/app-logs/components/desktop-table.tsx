...
export const DesktopTable = <T,>({ diffs, formatDisplayValue, copyToClipboard }: DesktopTableProps<T>) => {
    const t = useLang();

    if (diffs.length === 0) {
        return <p className='p-3 text-xs text-gray-500 dark:text-gray-400'>{t.helper('nothing_action')}</p>;
    }

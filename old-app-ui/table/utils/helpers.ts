 ...
 */
export function calculateColumnWidth<T extends BaseTableItem>(column: TableColumn<T>, data: T[]): number {
    const minWidth = 100;
    const maxWidth = 400;
    ...
    const maxContentLength = Math.max(
        ...data.slice(0, 100).map((item) => {
            ...
            const formattedValue = column.formatValue ? String(column.formatValue(item)) : formatCellValue(value);

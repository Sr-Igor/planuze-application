...
function TableColgroupComponent<T extends BaseTableItem>({ columns, selectable, hasActions }: TableColgroupProps<T>) {
    const getCellStyles = (column: TableColumn<T>) => {
        ...
        if (!column.width && !column.minWidth && !column.maxWidth) {
            ...
            if (column.width) {
                ...
                styles.width = typeof column.minWidth === 'number' ? `${column.minWidth}px` : column.minWidth;
            }
        }

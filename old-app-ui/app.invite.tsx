...
export const AppInvite = ({ invite, loading, onFeedback }: IAppInviteProps) => {
    ...
    const isExpired = invite.expire_date ? differenceInDays(new Date(invite.expire_date), new Date()) < 0 : false;

    return (
        <Card key={invite.id} className='relative flex h-18 flex-row justify-between gap-2 overflow-hidden p-0'>

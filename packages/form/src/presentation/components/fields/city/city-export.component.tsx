...
export function Cities({ value, onChange, className, country, state, disabled = false }: Readonly<ICityProps>) {
    const t = useLang();
    ...
    const selected = data.find((city) => city.value?.toLowerCase() === value?.toLocaleLowerCase()) || {
        label: value,
    };

...
export function States({ value, onChange, className, country, disabled = false }: Readonly<IStateProps>) {
    const t = useLang();
                        ...
                        <CommandGroup>
                            {data.map((state) => (
                                ...
                                >
                                    {state.label}
                                    <CheckIcon
                                        className={cn('ml-auto', value === state.value ? 'opacity-100' : 'opacity-0')}
                                    />
                                </CommandItem>
                            ))}
                        </CommandGroup>

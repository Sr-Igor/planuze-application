...
import { IOption, IUseInfinityProps } from '#/shared/types/select.types';

export function useInfinity<T>({
    cacheKey,
    queryParams = {},
    searchParam = 'search',
    ...
    const internalIndex = useInfiniteQuery<Pagination<T>>({
        ...
        initialPageParam: 1,
        enabled: !externalIndex && enabled && (enabledOnOpen ? open : true),
    });

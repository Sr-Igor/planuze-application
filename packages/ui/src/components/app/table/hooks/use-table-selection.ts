...
export function useTableSelection({ initialSelected: selectedItems = [], onSelectionChange }: UseTableSelectionProps) {
    // const [selectedItems, setSelectedItems] = useState<string[]>(initialSelected);

    const handleToggleItem = useCallback(
        (itemId: string, checked: boolean) => {
            const newSelection = checked ? [...selectedItems, itemId] : selectedItems.filter((id) => id !== itemId);
    ...
    const isItemSelected = useCallback((itemId: string) => selectedItems.includes(itemId), [selectedItems]);

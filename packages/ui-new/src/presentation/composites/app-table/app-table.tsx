...
import { useTableState, useWindowWidth } from "./hooks";
...
function TableComponent<T extends BaseTableItem>({
  ...
  const contextValue = useMemo(
    ...
    [tableState, state.loadingItems, state.expandedItems, events, expandable, size, variant, labels]
  );
      ...
      <TableProvider value={contextValue}>
        ...
        >
          ...
          >
            {tableState.isEmpty ? (
              <AppTableEmptyState
                message={emptyMessage || labels.emptyData || "No data available"}
                labels={labels}
              />
            ) : (
              <Table className={cn("min-w-full table-fixed", variantClasses.table)}>

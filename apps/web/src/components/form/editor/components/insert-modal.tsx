...
import { useModalCOnfig } from "../hooks/use-modal-configs";
import type { InsertModalProps } from "../types";
...
export const InsertModal: React.FC<InsertModalProps> = ({
  ...
  // Constante para tamanho máximo de imagem (2MB padrão)
  ...
  const handleConfirm = async () => {
    if (value.trim()) {
      if (type === "link") {
        // Para links, enviar objeto com URL e texto
        const linkData = {
          url: value.trim(),
          text: linkText.trim() || value.trim(), // Se não tiver texto, usar a URL
        };
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setValue(""); // Limpar URL quando selecionar arquivo
    }
  };
  ...
  const handleUrlChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (event.target.value.trim()) {
      setSelectedFile(null); // Limpar arquivo quando digitar URL
      if (fileInputRef.current) {
        fileInputRef.current.value = "";
      }
    }
  ...
  const handleOpenChange = (isOpen: boolean) => {
    onOpenChange(isOpen);
    // Se está fechando o modal, limpar estados e focar no editor
    if (!isOpen) {
      setValue("");
      setLinkText("");
      setSelectedFile(null);
      ...
      setTimeout(() => {
        // Dispara um evento customizado para que o editor possa se focar
        window.dispatchEvent(new CustomEvent("editor-focus-request"));
      }, 100);
    }
  };

  // Intercepta o ESC globalmente quando o modal está aberto
  useEffect(() => {

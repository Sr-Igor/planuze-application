...
export function Icons({ value, onChange, className, disabled = false }: IIconProps) {
  const t = useLang();
      ...
      return (
        <div style={{ ...style, display: "flex", alignItems: "center", height: 36 }}>
          ...
          >
            ...
            <span className="line-clamp-1 truncate text-sm font-normal capitalize">
              {icon.label}
            </span>
            <CheckIcon
              className={cn("ml-auto", value === icon.value ? "opacity-100" : "opacity-0")}
            />
          </CommandItem>
        </div>
      );

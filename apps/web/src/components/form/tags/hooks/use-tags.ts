...
export function useTags<T>({
    ...
    const items = useMemo(
        () =>
            formatter(data)?.filter(
                (item) =>
                    !value?.some((selectedItem) => selectedItem.title?.toLowerCase() === item.label?.toLowerCase())
            ) ?? [],
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (commandList && commandList.contains(target)) {
                return;
            }

            if (commandItem && commandItem.contains(target)) {
                return;
            }

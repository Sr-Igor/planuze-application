...
import { integration, integration_action, module } from "@/api/generator/types";
...
export const Permission = () => {
  ...
  const { data, handleState, companyView, userView, access, permissions } = useShow<integration>();
  ...
  return (
    <div className="flex flex-col gap-4 px-1 sm:px-5">
      {modules.map((module) => {
        ...
        const moduleActions =
          data?.integration_actions?.filter(
            (item: integration_action) => item.feature?.module_id === module.id
          ) || [];

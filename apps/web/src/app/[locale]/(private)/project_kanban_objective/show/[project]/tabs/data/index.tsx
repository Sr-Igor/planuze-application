...
export const Data = () => {
    ...
    const { update, store } = useProjectKanbanObjective({
        ...
        callbacks: {
            ...
            store: {
                onSuccess: (d) =>
                    router.replace(`/project_kanban_objective/show/${project}/${(d as project_kanban_objective).id}`)
            }
        }
    });
    ...
    return (
        <DataForm
            ...
            <Form
                {...formProps}
                hook={hook}
                className={cn('grid grid-cols-1 gap-3 max-md:flex max-md:flex-col max-md:p-2 lg:grid-cols-3')}
                onSubmit={(e) => {
                    e.preventDefault();

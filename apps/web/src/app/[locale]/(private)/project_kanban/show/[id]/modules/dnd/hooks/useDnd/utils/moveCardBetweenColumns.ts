...
export const moveCardBetweenColumns = ({
    ...
    if (targetColumn.limit && (targetColumn.project_kanban_cycle_cards?.length || 0) >= targetColumn.limit) {
        return {
            board: cycle!,
            changed: false
        };
    ...
    const newColumns = columns?.map((column) => {
        if (column.id === sourceColumnId) {
            return {
                ...column,
                project_kanban_cycle_cards: column.project_kanban_cycle_cards?.filter((card) => card.id !== cardId)
            };

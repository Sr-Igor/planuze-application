...
export const useCallers = ({ req, general }: IUseCallersProps): IUseCallersReturnProps => {
    return {
        column: {
            onSubmit: (data) => {
                if (general.state.column) {
                    req.services.column.update.mutate(data);
                } else {
                    const form = {
                        ...data,
        ...
        card: {
            ...
            onSubmit: (data, close) => {
                ...
                if (!general.state.card || general.state.forcedMode === 'store') {
                    const form = {
                        ...
                        project_kanban_cycle_column_id: data.project_kanban_cycle_column_id || general.state.columnId
                    };
            ...
            onTrash: () => req.services.card.trash,
            onRestore: (data) => {
                req.services.card.restore.mutate(data);
            }
        },
        cardType: {
            onSubmit: (data) => {

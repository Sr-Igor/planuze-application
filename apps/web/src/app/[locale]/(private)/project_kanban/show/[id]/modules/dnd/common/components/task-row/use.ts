...
export const useComponent = ({ mainTask, columns, index }: Pick<ITaskRowProps, 'mainTask' | 'columns' | 'index'>) => {
    ...
    const progressPercentage = allSubTasks.length > 0 ? (completedSubtasks / allSubTasks.length) * 100 : 0;

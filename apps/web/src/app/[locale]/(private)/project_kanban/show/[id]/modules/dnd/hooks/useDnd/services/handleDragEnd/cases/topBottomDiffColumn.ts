...
export const topBottomDiffColumn = ({
    ...
    if (activeColumnId !== actualColumnId) {
        ...
        const targetIndex = insertPosition?.columnId === actualColumnId ? insertPosition.index : undefined;
        ...
        if (changed) {
            ...
            if (card) {
                ...
                const newColumn = newBoard.project_kanban_cycle_columns?.find((col) => col.id === actualColumnId);
                const newOrder = newColumn?.project_kanban_cycle_cards?.findIndex((c) => c.id === activeId) || 0;

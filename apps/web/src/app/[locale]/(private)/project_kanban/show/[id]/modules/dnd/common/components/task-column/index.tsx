...
export const TaskRowColumn = ({
    ...
    const { setNodeRef, setTopDropRef, setBottomDropRef, shouldShowPlaceholder, shouldShowBottomPlaceholder } =
        useComponent({
            column,
            mainTaskId,
            insertPosition,
    ...
    return (
        <div className='border-border bg-sidebar mb-6 min-h-[120px] w-80 flex-shrink-0 border-t-3 border-r-3 border-b-3 p-1'>
            <div ref={setNodeRef} className='h-full'>
                <SortableContext items={subTasks.map((card) => card.id)} strategy={verticalListSortingStrategy}>
                    <div className='relative flex h-full min-h-[180px] flex-col pb-10'>
                        ...
                        {(subTasks.length > 0 || inCreating) && (
                            <div ref={setTopDropRef}>{<CardPlaceholder visible={shouldShowPlaceholder(0)} />}</div>
                        )}
                        ...
                        <div ref={setBottomDropRef}>{<CardPlaceholder visible={shouldShowBottomPlaceholder()} />}</div>

...
export const useForm = ({ disabled, item, open }: IUseFormProps) => {
    ...
    const defaultValues = useMemo<Partial<FormData>>(() => {
        ...
        return {
            ...
            project_kanban_cycle_column_id: columnExists ? item.project_kanban_cycle_column_id : undefined,
            project_kanban_cycle_card_type_id: cardTypeExists ? item.project_kanban_cycle_card_type_id : undefined,
    ...
    const fields = useMemo<Field<Partial<FormData>>[]>(
        ...
        [disabled, cycleOptions, columns, watches.cycle, watches.cardType, page.kanban?.project_id, item]
    );
    ...
    const validateAndResetDependentFields = useCallback(() => {
        ...
        if (watches.column && !columnExists) {
            ...
            const initialColumnExists = columns.some((col) => col.value === item?.project_kanban_cycle_column_id);
            if (
                initialColumnExists &&

...
const KanbanCardComponent = ({
    ...
    const { selected, setSelected, cardRef, cardType, tags, isVisible, isUnlinked, cardId } = useComponent({
        card,
        visibleCards,
        invisible,
        loading: externalLoading
    ...
    return (
        <Root
            ...
            <span className={cn('flex flex-col justify-between gap-1 transition-opacity duration-300 ease-in-out')}>
                <Header cardType={cardType} card={card} column={column} isUnlinked={isUnlinked} cardId={cardId} />
...
export const KanbanCard = memo(KanbanCardComponent, (prevProps, nextProps) => {
    return (
        ...
        prevProps.card.project_kanban_cycle_column_id === nextProps.card.project_kanban_cycle_column_id &&
        prevProps.card.project_kanban_cycle_card_type_id === nextProps.card.project_kanban_cycle_card_type_id &&

 ...
 * @returns
 */
export const complexity = ({ card, overData, cycle, updateInsertPosition }: IComplexityProps): boolean => {
        ...
        const siblingSubTasks =
            overColumn.project_kanban_cycle_cards?.filter((c) => {
                ...
                if (overCardMainTaskId === null) {
                    return c.card_id === null && c.id !== card?.id && !c.project_kanban_cycle_card_type?.principal;
                }

...
export const moveSubCardBetweenTaskColumns = ({
    ...
    const newColumns = cycle.project_kanban_cycle_columns.map((column) => {
        if (column.id === sourceColumnId && column.id === targetColumnId) {
            ...
            const sortedTargetSubCards = [...targetSubCards].sort((a, b) => (a.order || 0) - (b.order || 0));
            ...
            return {
                ...column,
                project_kanban_cycle_cards: column.project_kanban_cycle_cards?.filter((c) => c.id !== cardId)
            };
            ...
            const otherCards = existingCards.filter((c) => c.card_id !== targetMainTaskId && c.id !== cardId);

...
import { project_kanban_cycle_allocation } from "@repo/api/generator/types";
...
export const Allocation = ({ open, onOpenChange }: IAllocationProps) => {
  ...
  const filtered = useMemo(() => {
    if (!search) return data.profiles;

    return data.profiles.filter((item) => item.name?.toLowerCase().includes(search.toLowerCase()));
    ...
    <>
      ...
      >
        <DrawerContent>
          <DrawerHeader className="flex flex-row items-start justify-between border-b">
            <span>
              <DrawerTitle>{t("drawer.allocation.title")}</DrawerTitle>
              <DrawerDescription>{t("drawer.allocation.description")}</DrawerDescription>
            </span>
            <DrawerClose asChild>
          ...
          <div className="mb-4 flex items-center justify-between gap-2 px-4 py-4">
            <Input
              icon="Search"
              placeholder={t("drawer.allocation.placeholder")}
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
            <Permission permission={["store"]} feature="project_kanban_cycle">
              <Button size="sm" variant="secondary" onClick={() => state.allocation.form()}>
                <Plus />
              </Button>
            </Permission>
            ...
            <AppDropdownMenu
              ...
              groups={[
                {
                  title: t("drawer.allocation.actions.title"),
                  items: [
                    {
                      icon: "Plus",
                      label: t("drawer.allocation.actions.new_allocation"),
                      onClick: () => state.allocation.form(),
                    ...
                    {
                      icon: "Trash",
                      label: t("drawer.allocation.actions.trash"),
                      onClick: () => state.allocation.trash(),
              ...
              {filtered.length === 0 && (
                <div className="flex h-full min-h-[500px] flex-col items-center justify-center gap-2">
                  ...
                  <p className="text-muted-foreground text-md text-center font-semibold">
                    {t("drawer.allocation.no_results")}
                  </p>
                  <Permission permission={["store"]} feature="project_kanban_cycle">
                    <Button size="sm" variant="secondary" onClick={() => state.allocation.form()}>
                      <Plus />
                      {t("drawer.allocation.actions.new_allocation")}
                    </Button>
                  </Permission>
                </div>
              )}
      ...
      <Permission permission={["trash"]} feature="project_kanban_cycle">
        <AppDialog
          title={
            <span className="text-md flex items-center font-bold">
              ...
              {t("drawer.allocation.actions.trash")}
            </span>
          }
          open={general.isOpen("allocation", ["trash"])}
        ...
        >
          ...
          >
            <Trash<project_kanban_cycle_allocation>
              items={callers.allocation.onTrash()?.data || []}
              loading={loadings.allocation.restore || loadings.allocation.trash}
              showKeys={["profile"]}
              format={{
              ...
              actions={[
                {
                  label: t("drawer.allocation.actions.restore"),
                  icon: <ClockArrowDown className="h-4 w-4" />,
                  onClick: callers.allocation.onRestore,

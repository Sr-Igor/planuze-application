...
export const useCache = () => {
    ...
    const querySort = ({ data, orderKey = 'createdAt', orderValue = 'desc', dataType = 'date' }: IQuerySort) => {
    ...
    return {
        insertInIndex: ({ key, access = 'data', item, orderKey, orderValue, dataType, refetch = true }: ICache) => {
        ...
        updateInIndex: ({ key, access = 'data', item, orderKey, orderValue, dataType, refetch = true }: ICache) => {
            queryClient.setQueriesData({ queryKey: key }, (oldData: any) => {
                return {
                    ...oldData,
                    [access]: querySort({
        ...
        destroyInIndex: ({ key, access = 'data', item, orderKey, orderValue, dataType, refetch = true }: ICache) => {
            queryClient.setQueriesData({ queryKey: key }, (oldData: any) => {
                return {
                    ...oldData,
                    [access]: querySort({
        ...
        replaceShow: ({ key, item }: Omit<ICache, 'access' | 'orderKey' | 'orderValue' | 'refetch'>) => {
            queryClient.setQueryData(Array.isArray(key) ? key : [key], (old: any) => ({ ...(old || {}), ...item }));
        },
        ...
        filterAndReplaceInNestedArray: ({
            ...
            queryClient.setQueriesData({ queryKey: key }, (oldData: any) => {
                ...
                const filteredItems = (oldData?.[field] || []).filter((item: any) => item[filterKey] !== filterValue);
        ...
        setQueriesData: (queryKey: string | string[], updater: any) => {
            queryClient.setQueriesData({ queryKey: Array.isArray(queryKey) ? queryKey : [queryKey] }, updater);
        },

...
import { AppDialog, AppLogsModal, Icon } from "@repo/ui-new";
      ...
      <div className="flex items-center gap-2">
        ...
        {!isTrash && (
          <Permission permission={["trash"]}>
            <Button
              onClick={() => push(trashUrl || `/${path}/trash`)}
              disabled={isBusy}
              variant={"outline"}
            >
              <Trash2 />
              <span className="ml-2 hidden lg:inline-block">{t.helper("trash", vars)}</span>
            </Button>
          </Permission>
        )}

        {!isTrash && !hideCheckbox && (
          <Permission permission={["destroy", "many"]}>
            <Button
        ...
        {isTrash && !hideCheckbox && (
          <Permission permission={["restore", "many"]}>
            <Button
    ...
    <>
      <ListTemplate<T>
        ...
        table={{
          ...
          selected: perm.many && !!requests?.many && !hideCheckbox ? state.selected : undefined,
          loadingLines: state.loadingLines,
          events: {
            onSelectionChange:
              perm.many && !hideCheckbox ? (selected) => handleState({ selected }) : undefined,
            onFiltersChange: (newFilter) =>
              handleState({ filters: { ...state.filters, ...newFilter } as PaginationParams }),
            onRowDoubleClick: (item) => {
              if (!perm.update && !perm.show) return;
              redirect
                ? redirectWithBar
      ...
      {!isTrash && (
        <AppDialog
          ...
          footer={
            <Permission permission={state.mode === "destroy" ? ["destroy"] : ["destroy", "many"]}>
              <Button
                variant="destructive"
                onClick={() => {
      ...
      {isTrash && (
        <AppDialog
          ...
          footer={
            <Permission permission={state.mode === "restore" ? ["restore"] : ["restore", "many"]}>
              <Button
                variant="default"
                onClick={() => {
      ...
      {showLogs && (
        <AppLogsModal
          open={!!state.open && ["logs"].includes(state.mode)}

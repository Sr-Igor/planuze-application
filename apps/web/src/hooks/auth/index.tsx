...
import { getModule } from '../cookies/module';
import { getProfile } from '../cookies/profile';
import { getTwoAuth } from '../cookies/two_auth';
...
export const useAuth = () => {
    ...
    const { all: allModules, moduleId: moduleIdStore, profileId: profileIdStore, actions } = moduleState;

    const profileId = profileIdCookie || profileIdStore;
    const moduleId = moduleIdCookie || moduleIdStore;

    const profile = useMemo(() => user?.profiles?.find((p) => p.id === profileId), [user?.profiles, profileId]);
    ...
    const hasLevel = !!user?.id && !!user.confirmed && !!user?.profiles?.find((p) => p.id === profileId)?.level_id;
    const hasProfile = !!user?.id && !!user.confirmed && !!user?.profiles?.find((p) => p.id === profileId)?.active;

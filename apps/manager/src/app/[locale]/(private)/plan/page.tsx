...
import { Eraser, Plus, Users } from "lucide-react";
...
export default function Page() {
  ...
  const { actions } = useActions({ handleState, loadingLines: state.loadingLines });

  const { index } = useReq({ state, handleState });
  const { isBusy, isInitialState, push } = usePage({
    baseFilters: baseFilterParams,
    index,
    handleState,
    state,
  });
    ...
    <ListTemplate<plan>
      titlePage={
        <h1 className="mb-5 flex items-center gap-2 text-2xl font-bold max-sm:text-xl">
          <Users /> {t.page.plans("title")}
        </h1>
      }
      headerRight={
        <div className="flex items-center gap-2">
          <Button onClick={() => push("/plan/show")} disabled={isBusy}>
            <Plus />
            <span className="ml-2 hidden lg:inline-block">{t.page.plans("new")}</span>
          </Button>
        </div>
      }
      headerLeft={

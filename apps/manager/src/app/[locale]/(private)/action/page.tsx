...
import { Eraser, Pickaxe, Plus } from "lucide-react";
...
export default function Page() {
  ...
  const { index, store, update } = useReq({ state, handleState });
  ...
  return (
    <>
        ...
        headerRight={
          <div className="flex items-center gap-2">
            <Button onClick={() => handleState({ open: true, mode: "new", item: undefined })}>
              <Plus />
              <span className="ml-2 hidden lg:inline-block">{t.page.actions("new")}</span>
            </Button>
          </div>
        }
        headerLeft={
          <Button
            ...
            onClick={() => handleState({ filters: baseFilters })}
      ...
      <FormDialog
        modalTitle={t.page.actions(`modal.${state.mode!}.title`)}
        modalDescription={t.page.actions(`modal.${state.mode!}.description`)}
